...
function buscarLibros($biblioteca, $termino) {
    ...
    return array_filter($biblioteca, function ($libro) use ($termino) {
        ...
        return strpos($titulo, $termino) !== false || strpos($autor, $termino) !== false;
    });
}
...
{
    $reporte_biblioteca = array();
    $total_libros = count($biblioteca);
    $libros_prestados = 0;
    $autor_con_mas_libros = array();
    $cant_libros_por_genero = array();

    foreach ($biblioteca as $array) {
        if ($array['prestado']) {
            $libros_prestados++;
        }
        array_push($cant_libros_por_genero, $array['genero']);
        array_push($autor_con_mas_libros, $array['autor']);
    }

    $reporte_biblioteca = [
        "autor_con_mas_libros" => array_count_values($autor_con_mas_libros),
        "cant_libros_por_genero" => array_count_values($cant_libros_por_genero),
        "total_libros_prestados" => $libros_prestados,
        "total_libros" => $total_libros,
    ];
    return $reporte_biblioteca;
}
